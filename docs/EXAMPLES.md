...
- [Детальная информация об устройстве](#детальная-информация-об-устройстве)
- [Команды устройству](#команды-устройству)
- [События и история](#события-и-история)
- [Описание типов событий](#описание-типов-событий)
- [Трек устройства](#трек-устройства)
- [Местоположение устройства](#местоположение-устройства)
...
Метод `details()` возвращает полную информацию: состояние авто, датчики,
уровень GSM, версию прошивки, доступные функции и управление.

```php
$info = $api->devices()->details($deviceId);
$data = $info['data'] ?? [];

echo 'Алиас:        ', $data['alias'] ?? '?', "\n";
echo 'АКБ:          ', $data['battery'] ?? '?', " В\n";
echo 'GSM:          ', $data['gsm_lvl'] ?? '?', "\n";
echo 'Прошивка:     ', $data['fw_version'] ?? '?', "\n";
echo 'Охрана:       ', ($data['car_state']['arm'] ?? false) ? 'да' : 'нет', "\n";
echo 'Двигатель:    ', ($data['car_state']['run'] ?? false) ? 'работает' : 'остановлен', "\n";
```
...
$desc = $api->devices()->eventType(307);
echo $desc['eventDescriptions'][0]['desc']; // "Тревога по датчику ручного тормоза"

// Вся библиотека событий
$all = $api->devices()->eventTypes();
foreach ($all['eventDescriptions'] as $event) {
    echo $event['code'], ' — ', $event['desc'], PHP_EOL;
}
```
...
Метод `position()` возвращает последнее зафиксированное местоположение:

```php
$pos = $api->devices()->position($deviceId);

$p = $pos['device']['position'] ?? [];
echo 'Широта: ', $p['lat'] ?? '?', "\n";
echo 'Долгота: ', $p['lon'] ?? '?', "\n";
echo 'Время:  ', $p['ts'] ?? '?', "\n";
```

> Метод устарел в StarLine API. Рекомендуется использовать `state()` вместо него, но endpoint всё ещё работает.
